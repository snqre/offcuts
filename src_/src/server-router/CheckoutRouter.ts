import type { StripeE } from "src_/src/server-class/_";
import type { StoreE } from "src_/src/server-class/_";
import { Router } from "express";
import { Result } from "reliq";
import { Ok } from "reliq";
import { Err } from "reliq";
import { Stripe } from "src_/src/server-class/_";
...
export function CheckoutRouter(_store: Store): CheckoutRouterR {
    /** @constructor */ {
        let routerR: Result<Router, unknown> = 
        wrapR(() => 
            Router()
                .post("/checkout", async (rq, rs) => {
                    ...
                    let stripe: Stripe; { 
                        let $ = Stripe(stripeApiKey); 
                        if ($.err()) {
                            rs.status(500).send($.val());
                            return;
                        }; 
                        stripe = $.unwrapSafely(); 
                    }
